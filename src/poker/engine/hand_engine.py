...
from poker.logging.state_snapshot import format_state_snapshot
...
def play_hand(
    ...
    if _count_active_players(state) <= 1:
        return _finish_hand(state, bots, deck, logger)
...
def _post_blinds(state: GameState, logger: Logger) -> GameState:
    ...
    new_stack_sb = sb_player.stack - sb_amount
    players_list[sb_seat] = (
        sb_player.with_stack(new_stack_sb)
        .with_committed_this_street(sb_amount)
        .with_committed_this_hand(sb_amount)
    )
    logger.log_event(
        BlindPosted(
            hand_number=state.hand_number,
    ...
    new_stack_bb = bb_player.stack - bb_amount
    players_list[bb_seat] = (
        bb_player.with_stack(new_stack_bb)
        .with_committed_this_street(bb_amount)
        .with_committed_this_hand(bb_amount)
    )
    logger.log_event(
        BlindPosted(
            hand_number=state.hand_number,
...
def _finish_hand(
    state: GameState, bots: dict[int, Bot], deck: Deck, logger: Logger
) -> GameState:
    ...
    state = replace(state, street=Street.SHOWDOWN)
    _log_street_snapshot(state, logger)
...
def _rebuild_pots(state: GameState) -> GameState:
    """Rebuild the pots list based on current committed_this_hand amounts.

    This ensures the UI always has current pot information. Pots are calculated
    from the chips each player has committed to the hand, excluding folded players
    from eligibility.
    ...
    Returns:
        Updated game state with pots rebuilt.
    """
    from poker.state.pot_manager import build_pots

    # Build committed and folded sets
    committed = {i: state.players[i].committed_this_hand for i in range(len(state.players))}
    folded = {i for i, p in enumerate(state.players) if p.has_folded or p.is_eliminated}

    # Rebuild pots
    pots = build_pots(committed, folded)

    return replace(state, pots=pots)
