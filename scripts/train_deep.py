#!/usr/bin/env python
"""Train a deep Q-learning neural network model."""
...
def main() -> None:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Train a deep neural network poker agent")
    parser.add_argument("--data", type=str, default="data/selfplay.npz", help="Input data file")
    parser.add_argument("--out", type=str, default="models/deep_q.pt", help="Output model file")
    ...
    # Train model
    print(f"\nTraining DeepQModel (hidden={args.hidden}, lr={args.lr}, epochs={args.epochs})...")
    model = DeepQModel(num_actions=7, hidden_size=args.hidden, lr=args.lr)
    model.fit(X_train, actions_train, rewards_train, masks_train, epochs=args.epochs, batch_size=args.batch_size)
