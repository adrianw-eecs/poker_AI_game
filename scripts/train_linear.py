#!/usr/bin/env python
"""Train a linear regression Q-learning model."""
...
def main() -> None:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Train a linear regression poker agent")
    parser.add_argument("--data", type=str, default="data/selfplay.npz", help="Input data file")
    parser.add_argument("--out", type=str, default="models/linear_q.pkl", help="Output model file")
    ...
    # Train model
    print(f"\nTraining LinearQModel (alpha={args.alpha})...")
    model = LinearQModel(num_actions=7, alpha=args.alpha)
