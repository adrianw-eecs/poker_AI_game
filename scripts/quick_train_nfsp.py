#!/usr/bin/env python
"""Quick NFSP training test (15 min): 10K episodes to validate implementation."""
...
def evaluate(model: NFSPModel, num_players: int, num_hands: int = 100) -> float:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Quick NFSP training test (10K episodes)")
    parser.add_argument("--num-players", type=int, default=2)
    parser.add_argument("--episodes", type=int, default=10_000)
    parser.add_argument("--eval-every", type=int, default=2_000)
    parser.add_argument("--generation", type=int, default=0)
    parser.add_argument("--save-path", type=str, default="models/nfsp_quick_test.pt")
    args = parser.parse_args()

    print(f"\n{'='*70}")
    print(f"Quick NFSP Training Test: {args.episodes:,} episodes")
    print(f"{'='*70}")
    ...
    for episode in range(args.episodes):
        ...
        if episode % 200 == 0 or episode == args.episodes - 1:
            if episode % args.eval_every == 0:
                eval_reward = evaluate(model, args.num_players, num_hands=50)
            else:
                eval_reward = None
            ...
            eval_str = f"{eval_reward:7.4f}" if eval_reward is not None else "    n/a"
            print(
                f"Ep {episode:6d}/{args.episodes:6d} | q_loss={q_str} | policy_loss={p_str} "
                f"| eval={eval_str} | elapsed={elapsed:6.1f}s | ETA={eta:6.1f}m"
            )
    ...
    print(f"Avg time per episode: {np.mean(episode_times):.3f}s")
