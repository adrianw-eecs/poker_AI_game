#!/usr/bin/env python
"""Test SD-CFR Scenario 1: 4 players, 10 hands, rebuy ON."""
...
def main():
    ...
    event_logger = GameLogger(games_dir / f"sdcfr_s1_{ts}.jsonl")
    transcript_logger = SessionTextLogger(games_dir / f"sdcfr_s1_{ts}.txt")
    ...
    # Create bots
    bots = {
        0: SDCFRBot(name="SDCFRBot", model=sdcfr_model),
    ...
    # Print basic results
    print("\n" + "=" * 80)
    print("SD-CFR SCENARIO 1 — 4 players | 10 hands | rebuy ON")
    print("=" * 80)
    ...
    log_file = games_dir / f"sdcfr_s1_{ts}.jsonl"
    if log_file.exists():
        seat_to_bot = {
            0: "SDCFRBot",
            1: "RandomBot1",
