...
class PopulationManager:
    ...
    def load_generation(self, gen_id: int) -> NFSPModel:
        """Load a saved generation model (cached to avoid reloading).
        ...
        if gen_id in self._model_cache:
            return self._model_cache[gen_id]
        ...
        self._model_cache[gen_id] = model
        return model
    ...
    def get_opponent_roster(self, current_gen: int, num_opponents: int) -> list:
        ...
        if self_play_count > 0 and current_gen > 0:
            try:
                ...
                # Previous generation not saved yet, use RandomBot instead
                for i in range(self_play_count):
                    opponents.append(RandomBot(name=f"RandomBot_fallback_{i}", seed=250 + i))
        ...
        for i in range(pop_count):
            if current_gen >= 2:
                ...
                try:
                    ...
                    # Historical generation not available, use RandomBot
