...
from poker.logging.state_snapshot import format_action_log_line
...
class BettingRound:
    ...
    def __init__(
        ...
        self.action_count_this_street: int = 0
    ...
    def run(self) -> GameState:
        ...
        # Ensure action starts with a player who can act (not folded/eliminated/all-in)
        self.state = self._set_initial_action_on_seat(self.state)
        ...
        while not self._round_is_closed():
            ...
            committed_before = self.state.players[seat].committed_this_hand
            ...
            self.state = rebuild_pots(self.state)
            chips_added = (
                self.state.players[seat].committed_this_hand - committed_before
            )
            pot_after = sum(p.amount for p in self.state.pots)
            self.action_count_this_street += 1
            ...
            log_line = format_action_log_line(
                self.state, seat, action, self.action_count_this_street, chips_added
            )
            self.logger.log_event(
                ActionTaken(
                    ...
                    pot_after=pot_after,
                    action_num=self.action_count_this_street,
                    log_line=log_line,
                )
            )

        # Round is closed - clear action_on_seat and committed_this_street
        final_state = replace(self.state, action_on_seat=None)
        # Keep committed_this_street as-is (will be used for pot calculation and reset on next street)
        return final_state
    ...
    def _apply_action(self, state: GameState, seat: int, action: Action) -> GameState:
        ...
        if action.type == ActionType.FOLD:
            ...
            # Mark as all-in if stack is depleted OR if call amount < bet to call (incomplete call)
            if new_stack == 0 or action.amount < state.current_bet_to_call:
    ...
    def _set_initial_action_on_seat(self, state: GameState) -> GameState:
        """Set action to first non-folded, non-eliminated, non-all-in player from action_on_seat.

        This is used when moving to a new street where all folded players should be
        permanently excluded from acting.

        Args:
            state: Current GameState.

        Returns:
            Updated GameState with action_on_seat properly set.
        """
        ...
        # Find the first player who can act starting from the current seat
        for _ in range(num_players):
            player = state.players[current_seat]

            if not player.has_folded and not player.is_eliminated and not player.is_all_in:
                return replace(state, action_on_seat=current_seat)
