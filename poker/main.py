...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Bot types for each seat: random, human, flop_bot, nfsp_bot (default: all random)",
    )
    parser.add_argument(
        "--nfsp-model",
        type=str,
        default=None,
        help="Path to NFSP model checkpoint (.pt file) for nfsp_bot. If not specified, uses models/nfsp.pt",
    )
    parser.add_argument(
        "--rake",
        type=float,
        default=5.0,
        help="Rake percentage (0.0 to 10.0, default: 5.0)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Random seed for reproducibility. If not set, a random seed is used.",
    )
    parser.add_argument(
        "--run-it-twice",
        action="store_true",
    ...
    try:
        ...
        transcript_logger = SessionTextLogger(games_dir / f"session_{ts}.txt", seed=args.seed)
        ...
        for seat, bot_type in enumerate(bot_types[: args.num_players]):
            if bot_type == "human":
                ...
                model = NFSPModel()
                model_path = args.nfsp_model or "models/nfsp.pt"
                ...
                # Pass seed to RandomBot for reproducibility
                bots[seat] = RandomBot(name=f"RandomBot{seat + 1}", seed=args.seed)
        ...
        # Run session
        rng = RNG(seed=args.seed)  # Create RNG for deck shuffling with optional seed
        ...
        # Format final results for both console and log file
        results_lines = [
            "",
            "=" * 60,
            "FINAL RESULTS",
        ...
        for seat in sorted(results.keys()):
            ...
            line = f"{bot_name:20} {stack:6} ({change_str:>6})"
        ...
        # Log final results to transcript file
        transcript_logger.log_text("\n".join(results_lines))
